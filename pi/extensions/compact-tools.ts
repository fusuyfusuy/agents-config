...
function expandHint(theme: any): string {
  return theme.fg("muted", ` [ctrl+o expand]`);
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    renderCall(args, theme, _context) {
      ...
      text += theme.fg("accent", truncated(cmd, CALL_CHARS));
    ...
    renderResult(result, { expanded, isPartial }, theme, _context) {
      ...
      return new Text(text + prettyLines(output.replace(/\n*exit code: \d+$/m, ""), theme), 0, 0);
    },
  });
  ...
  pi.registerTool({
    ...
    renderCall(args, theme, _context) {
      ...
      text += theme.fg("accent", `"${truncated(args.pattern, CALL_CHARS)}"`);
