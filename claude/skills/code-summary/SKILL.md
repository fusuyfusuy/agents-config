...
| Who imports / is imported by X | `agent-ctx dump --focus X` (detail + graph neighbors) |
...
- Orientation ("what's in this repo?") — `agent-ctx dump --focus <area>` instead of walking the tree.
...
- Semantics beyond grep/ast (then read the 2–3 files `agent-ctx` ranked as core).
