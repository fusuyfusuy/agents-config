...
function installInlineFooter(ctx: any, sentence: string): void {
	...
	// avoid reinstalling every keystroke if same sentence; but we need to refresh width
	try {
		ctx.ui.setFooter((tui: any, theme: any, footerData: any) => {
			...
			return {
				...
				render(width: number): string[] {
					// Recompute footer like original but inject sentence next to pwd
					// Copy minimal logic from FooterComponent — keep it light
					const path = require("node:path") as typeof import("node:path");
					let pwd = "";
					try {
						...
						if (home) {
							...
							const inside =
								...
								!path.isAbsolute(rel);
							pwd = inside ? (rel === "" ? "~" : `~${path.sep}${rel}`) : cwd;
						} else pwd = cwd;
					} catch {
						pwd = process.cwd();
					}
					...
					// inject sentence
					if (inlineSentenceForFooter) {
						const avail = Math.max(20, width - pwd.length - 3);
						let s = inlineSentenceForFooter;
						if (s.length > avail) s = s.slice(0, Math.max(0, avail - 3)) + "...";
						pwd += ` • ${s}`;
					}
					// Lazy import helpers
					let truncateToWidth: any, visibleWidth: any;
					try {
						const tuiMod = require("@earendil-works/pi-tui") as any;
						truncateToWidth = tuiMod.truncateToWidth;
						visibleWidth = tuiMod.visibleWidth;
					} catch {
					...
					// Second line: stats from session
					let input = 0,
						output = 0,
						cost = 0;
					try {
					...
					let stats = `↑${fmt(input)} ↓${fmt(output)} $${cost.toFixed(3)}`;
					try {
						const cu: any = ctx.session?.getContextUsage?.() ?? null;
						if (cu && typeof cu.percent === "number")
							stats += ` ${cu.percent.toFixed(1)}%`;
					} catch (_e) {
						void _e;
					}
					const modelName = ctx.model?.id ?? "no-model";
					const leftW = visibleWidth ? visibleWidth(stats) : stats.length;
					const rightW = visibleWidth ? visibleWidth(modelName) : modelName.length;
					let statsLine = stats;
					if (leftW + 2 + rightW <= width) {
						const pad = " ".repeat(width - leftW - rightW);
						statsLine = stats + pad + modelName;
					}
					const dimStats = theme.fg("dim", statsLine);
					// third line: extension statuses
					let extraLine: string | null = null;
					try {
						const statuses: Map<string, string> =
							footerData.getExtensionStatuses?.() ?? new Map();
						// filter out our own footer key to avoid echo
						const filtered = Array.from(statuses.entries())
							.filter(([k]) => k !== FOOTER_KEY)
							.map(([, v]) =>
								String(v)
									.replace(/[\r\n\t]/g, " ")
									.trim(),
							...
							.filter(Boolean);
						if (filtered.length) extraLine = filtered.join(" ");
					} catch (_e) {
						void _e;
					}
					...
					if (extraLine) {
						const t = truncateToWidth
							? truncateToWidth(extraLine, width, theme.fg("dim", "..."))
							: extraLine;
						lines.push(t);
					}
					return lines;
