// timestamps — inline HH:MM:SS local time for prompts and tool calls
//
// Shows a dim timestamp before every user prompt, assistant block, and tool
// execution row. Always on, composes with compact-tools.ts (patches the
// container rather than re-registering tools), and falls back to tool wrapping
// if the prototype patch cannot be applied.
//
...
function findPiRoot(): string | null {
	...
	// 2) common global install locations (fallback for the hardcoded nvm path check)
	candidates.push(
		"/home/devhax/.nvm/versions/node/v24.16.0/lib/node_modules/@earendil-works/pi-coding-agent",
	);
	try {
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("tool_execution_start", async (event) => {
		toolStart.set((event as any).toolCallId, Date.now());
	});

	// Also capture tool_call (LLM emitted the call, execution may be queued)
	...
	(async () => {
		...
		try {
			...
			if (root) {
				...
				const mod: any = await import(url);
				const Ctor = mod.ToolExecutionComponent;
				if (Ctor && !Ctor.prototype.__timestampsPatched) {
					const origUpdateDisplay = Ctor.prototype.updateDisplay;
					Ctor.prototype.updateDisplay = function (this: any) {
						origUpdateDisplay.call(this);
						// Determine timestamp for this tool call
						const id: string | undefined = this.toolCallId;
						let ts: number | undefined = id ? toolStart.get(id) : undefined;
						if (!ts) ts = Date.now();
						const dur = Date.now() - ts;
						const isDone = !!this.result;
						const timePart = dim(`[${formatTime(ts)}]`);
						const durPart =
							isDone && dur > 500 ? dim(` (+${formatDuration(dur)})`) : "";
						// Build stamp line
						// Use Text with no bg so it sits above the boxed content
						const stampText = `${timePart}${durPart} ${muted(this.toolName ?? "tool")}`;
						// Insert as first child of the component (above contentBox/selfRenderContainer)
						// But Container's children already contain Spacer(1) + contentBox/selfRenderContainer + images
						// We want the stamp to appear right above the tool box. Easiest: prepend a Text
						// as the first child. To keep Spacer from adding blank before stamp, we insert stamp
						// before everything, or replace the Spacer with stamp+spacer.
						// Check if already stamped for this id+state
						const stampKey = `${id}:${isDone ? "done" : "pending"}:${ts}`;
						if (this.__timestampsKey === stampKey) return;
						this.__timestampsKey = stampKey;
						// Remove prior stamp if present (first child is our prior stamp)
						if (this.__timestampsStamp) {
							this.removeChild(this.__timestampsStamp);
							this.__timestampsStamp = null;
						}
						const stamp = new Text(stampText, 0, 0);
						this.__timestampsStamp = stamp;
						// Insert at index 0 (before Spacer)
						// Container has no insertAt, so rebuild order: save children, clear, add stamp, re-add
						const existing = [...this.children];
						this.clear();
						this.addChild(stamp);
						for (const c of existing) this.addChild(c);
			...
			// eslint-disable-next-line no-console
			console.error("[timestamps] ToolExecution patch failed", e);
		}

		// If none of the patches succeeded, fall back to wrapping built-in tools
		// (this is the compact-tools style, but with timestamp prefix). This
		// keeps timestamps working even on a pi build where the component files
		// moved.
		let needFallback = false;
		try {
			const root = findPiRoot();
			if (!root) needFallback = true;
		} catch (_e9) {
			void _e9;
			needFallback = true;
		}
		if (needFallback) {
			installToolWrapping(pi, toolStart, dim, muted);
		}
	})();

	// If prototype patching is working, the fallback is not needed; we keep the
	// toolStart map alive for the patch. If it failed, the IIFE above will call
	// installToolWrapping.
}

// ---------------------------------------------------------------------------
// Fallback: re-register built-in tools with timestamp-prefixed renderers.
// Only used when component patching is unavailable (e.g. pi file layout drift).
// ---------------------------------------------------------------------------
