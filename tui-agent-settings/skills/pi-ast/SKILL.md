...
soul_impact({ action: "cochanges", file: "tui-agent-settings/skills/agent-context/agent-ctx" })
...
- `soul_find` caps `cochange` to top-5 for speed; full `PageRank damping 0.85 + trigram WITHOUT ROWID` is `ponytail:` to `agent-ctx` / `pi-lens`.
